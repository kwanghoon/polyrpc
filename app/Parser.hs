...
parserSpec = ParserSpec
  {
    ...
    [
      ...
      {- Location -}
      rule "Location -> identifier" ( \rhs -> return $ toASTLocation (Location (getText rhs 1)) ),
      ...
      rule "AtomicType -> identifier" ( \rhs -> return $ toASTType (TypeVarType (getText rhs 1)) ),
      ...
      rule "IdTypeLoc -> identifier : Type @ Location" (
        \rhs -> return $ toASTIdTypeLoc (getText rhs 1, Just (fromASTType (get rhs 3)), fromASTLocation (get rhs 5)) ),
      ...
      rule "Expr -> Expr { Identifiers }" (
        \rhs -> return $ toASTExpr (singleLocApp (LocApp (fromASTExpr (get rhs 1)) Nothing (map Location (fromASTIdSeq (get rhs 3))))) ),
      ...
      rule "CompEqNeq -> CompEqNeq == Comp" (  -- Assume EqIntPrimOp, which may change to EqBoolOp or EqStringOp later
        \rhs -> return $ toASTExpr (Prim EqIntPrimOp [] [] [fromASTExpr (get rhs 1), fromASTExpr (get rhs 3)]) ),
